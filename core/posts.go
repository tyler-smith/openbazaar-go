...
func (n *OpenBazaarNode) extractpostData(post *pb.SignedPost) (postData, error) {
	...
	postLength := len(post.Post.Post.Post)
	if postLength > PostLength {
...
func validatePost(post *pb.Post) (err error) {
	...
	// Post
	if len(post.Post.Post) > PostMaxCharacters {
		return fmt.Errorf("Post is longer than the max of %d characters", PostMaxCharacters)
	}
	if len(post.Post.Images) > MaxListItems {
		return fmt.Errorf("Number of post images is greater than the max of %d", MaxListItems)
	}
	for _, img := range post.Post.Images {
